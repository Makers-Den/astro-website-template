...
import {
  ...
  OPENAI_API_KEY,
  STORYBLOK_LLM_CONTEXTS_FOLDER_ID,
...
export const POST: APIRoute = async ({ request, url }) => {
  try {
    ...
    if (contextStories && contextStories.length === 1) {
      await updateContextStory(contextStories[0].id, story.id, story.full_slug, markdownContent);
...
async function createContextStory(sourceStoryId: number, sourceStorySlug: string, markdownContent: string) {
  ...
  const createResponse = await fetch(`https://mapi.storyblok.com/v1/spaces/${spaceId}/stories`, {
    method: 'POST',
    headers: {
...
async function updateContextStory(
  ...
  const response = await fetch(`https://mapi.storyblok.com/v1/spaces/${spaceId}/stories/${contextStoryId}`, {
    method: 'PUT',
    headers: {
      Authorization: managementToken,
      'Content-Type': 'application/json',
